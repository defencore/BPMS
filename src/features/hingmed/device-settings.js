...
export function buildDeviceConfiguration(read) {
    ...
    const steps = [
        step(0x43, uint16(read.integer('max-cuff-pressure', 140, 300))),
    ...
    steps.push(
        step(0x47, [0]),
        step(0x48, read.time('awake-begin')),
        step(0x49, [read.integer('awake-interval', 1, 255)]),
        step(0x4A, read.time('awake-end')),
        step(0x4B, [read.integer('asleep-interval', 1, 255)]),
        step(0x58, [specials.length ? 1 : 0])
    );

    for (const period of specials) {
        steps.push(
            step(period.begin, read.time(`${period.prefix}-begin`)),
            step(period.end, read.time(`${period.prefix}-end`)),
            step(period.interval, [read.integer(`${period.prefix}-interval`, 1, 255)])
        );
    }
